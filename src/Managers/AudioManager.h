...
public:
    ...
    bool init()
    {
        ...
        initialized = true;
        std::cout << "[AudioManager] Initialized successfully with 16 channels\n";
        return true;
    }
    ...
    bool loadMusic(const std::string &id, const std::string &filePath)
    {
        ...
        std::string fullPath = "assets/Audio/" + filePath;
    ...
    bool loadSound(const std::string &id, const std::vector<std::string> &filesPaths)
    {
        ...
        for (auto &filePath : filesPaths)
        {
            ...
            if (!sound)
            {
                ...
                std::cerr << "[AudioManager] failed to load sound at: " << fullPath << "\n";
                return false;
            }
    ...
    bool hasLoadMusic(const std::string& id){
        return musics.find(id) != musics.end();
    }


    bool hasLoadSound(const std::string& id){
    ...
    void setMusicVolume(int volume)
    {
        ...
        musicVolume = SDL_clamp(volume, 0, 128);
    }

    void setSoundVolume(int volume)
    {
        soundVolume = SDL_clamp(volume, 0, 128);
    }
